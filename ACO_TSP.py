...
class AntColonyOptimization:
    def __init__(self, distances, n_ants, n_iterations, decay_factor=0.5, alpha=1, beta=1, q0=0.5):
        ...
        self.pheromone = [[1 / (self.n_cities * self.n_cities)
                           for _ in range(self.n_cities)] for _ in range(self.n_cities)]
        self.best_path = None
    ...
    def choose_next_city(self, current_city, visited):
        unvisited_cities = [city for city in range(
            self.n_cities) if not visited[city]]
        if not unvisited_cities:
            return current_city

        probabilities = []
        for city in unvisited_cities:
            probabilities.append((city, self.pheromone[current_city][city] ** self.alpha * (
        ...
        if random.uniform(0, 1) < self.q0:
            ...
            probabilities = [(p[0], p[1] / total_probability)
    ...
    def update_pheromone(self, paths):
        delta_pheromone = [
            [0 for _ in range(self.n_cities)] for _ in range(self.n_cities)]
        for path in paths:
        ...
        for i in range(self.n_cities):
            for j in range(self.n_cities):
                self.pheromone[i][j] = (
                    1 - self.decay_factor) * self.pheromone[i][j] + delta_pheromone[i][j]
